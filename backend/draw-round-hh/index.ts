...
const MAX_PAYOUT = 100_000_000_000;
...
serve(async (req) => {
  ...
  try {
    ...
    for (const round of expiredRounds) {
      ...
      // 1. Обрізаємо Total Pool вниз до 4 знаків після коми (Math.floor)
      ...
      // 2. Рахуємо призи від безпечного пулу
      let prize = participants.length === 1 ? safeTotalPool : safeTotalPool * WINNER_SHARE;

      if (participants.length > 1 && prize < winnerStake) {
        prize = Math.min(winnerStake, safeTotalPool);
      }

      try {
        validatePayout({ winner, participants, payout: prize, totalPool: safeTotalPool, alreadyPaid: !!round.tx_hash_payout, maxAllowed: prize });
      } catch (secErr) {
      ...
      try {
        ...
        const prizeRaw = parseUnits(prize.toFixed(HH_DECIMALS), HH_DECIMALS);
        const dataSuffix = getBuilderDataSuffix();

        const hhRaffleVaultAddress = (Deno.env.get("HH_RAFFLE_VAULT_ADDRESS") || "0x3bdF461984142C473F2185B4F0F64a918B8ce49b") as `0x${string}`;
        
        // 3. Рахуємо комісію на основі безпечного пулу
        const fee = safeTotalPool - prize;
...
async function ensureNextRound(now: Date) {
  const { data: existing } = await supabase.from("rounds").select("id").in("status", ["open", "spinning"]).eq("currency", "HH").limit(1);
