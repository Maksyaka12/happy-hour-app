...
export function StakingSection() {
  ...
  const [activeSubTab, setActiveSubTab] = useState('staking') // 'staking' or 'checklist'
  ...
  // Fetch real stats from Supabase for checklist if connected
  useEffect(() => {
    if (!address || !isConnected) return

    const fetchUserHistoryStats = async () => {
      try {
        ...
        console.warn('Error fetching user stats from db, using simulated data:', err)
      }
    }

    fetchUserHistoryStats()
  }, [address, isConnected])
  ...
  // Format balances (fall back to simulated values if blockchain reads are not fully active or configured)
  ...
  const handleApprove = async () => {
    ...
    // Simulate web3 flow with premium overlays
    setTimeout(() => {
      setTxStep('approve_pending')
      setTimeout(() => {
        // Set simulated allowance to unlimited/max
  ...
  const handleUnstake = async () => {
    ...
    setTimeout(() => {
      setTxStep('action_pending')
      setTimeout(() => {
        // Unstaking moves tokens into the 3-day cooldown queue
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            transition: 'all 0.2s',
            boxShadow: activeSubTab === 'staking' ? '0 4px 12px rgba(0,82,255,0.15)' : 'none'
          }}
        >
          🥩 Staking & Hold
        </button>
        <button
          ...
          style={{
            ...
            padding: '10px 12px',
            borderRadius: 12,
            border: activeSubTab === 'checklist' ? 'none' : '1px solid rgba(255,255,255,0.8)',
            background: activeSubTab === 'checklist' 
              ? 'linear-gradient(135deg, #0052FF 0%, #3B82F6 100%)' 
              : 'rgba(255, 255, 255, 0.6)',
            color: activeSubTab === 'checklist' ? '#fff' : '#717886',
            fontWeight: 850,
            fontSize: 12.5,
            cursor: 'pointer',
            transition: 'all 0.2s',
            boxShadow: activeSubTab === 'checklist' ? '0 4px 12px rgba(0,82,255,0.15)' : 'none'
          }}
        >
          📋 Airdrop Checklist
        </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div>
                <h3 style={{ fontSize: 15, fontWeight: 800, color: '#0A0B0D' }}>💼 Wallet Hold Rewards</h3>
                <p style={{ fontSize: 11, color: '#717886', marginTop: 2 }}>Earn passive HP simply by holding $HH</p>
              </div>
              <span style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div>
                <h3 style={{ fontSize: 15, fontWeight: 800, color: '#0A0B0D' }}>🥩 Lock Staking Rewards</h3>
                <p style={{ fontSize: 11, color: '#717886', marginTop: 2 }}>Lock tokens on contract for double HP yield</p>
              </div>
              <span style={{
          ...
          {pendingWithdrawals.length > 0 && (
            <div style={{
              ...
              <h3 style={{ fontSize: 14, fontWeight: 800, color: '#0A0B0D', marginBottom: 12 }}>⏳ Pending Withdrawals</h3>
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 15, fontWeight: 800, color: '#0A0B0D', marginBottom: 12 }}>🏆 Top Stakers Community</h3>
        ...
        <div style={{
          ...
          {checklistPercentage === 100 ? (
            <div style={{
              ...
              animation: 'tickPulse 2s infinite ease-in-out'
            }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: 12.5, color: '#717886', lineHeight: 1.5, marginBottom: 20 }}>
              {txStep === 'approve_signing' && 'Please sign the one-time unlimited approval in your wallet to enable staking.'}
              {txStep === 'approve_pending' && 'Approving infinite spending limit on Base Network...'}
